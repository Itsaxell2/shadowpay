...
app.post("/links/:id/pay", paymentLimiter, async (req, res) => {
  ...
  try {
    // CRITICAL: Deposit to Privacy Cash pool via embedded relayer
    // Using embedded relayer instead of external service call
    // This ensures:
    ...
    console.log(`💳 Initiating REAL devnet payment via embedded relayer...`);
    ...
    // Call embedded deposit function directly
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      // CRITICAL: Withdraw from Privacy Cash pool via embedded relayer
      // Using embedded relayer instead of external service call
      // This ensures:
      ...
      console.log(`💸 Initiating REAL devnet withdrawal via embedded relayer...`);
...
/* ─────────────────────── RELAYER (EMBEDDED) ─────────────────────── */
// TEMP FIX: Embed relayer functionality directly in backend
// This eliminates the need for separate relayer service deployment
// For production, consider deploying relayer separately for better separation
...
app.post("/deposit", async (req, res) => {
  try {
    ...
    console.log(`💰 [EMBEDDED RELAYER] Depositing ${lamports / 1e9} SOL to Privacy Cash...`);
    ...
    console.log(`📋 Verify on-chain: https://solscan.io/tx/${result.tx}?cluster=devnet`);
    ...
    console.error("❌ [EMBEDDED RELAYER] Deposit error:", err);
    res.status(500).json({ error: err.message });
  }
});
...
app.post("/withdraw", async (req, res) => {
  try {
    ...
    console.log(`💸 [EMBEDDED RELAYER] Withdrawing ${lamports / 1e9} SOL to ${recipient}...`);

    const result = await withdrawSOL({ recipient, lamports, referrer });
...
app.listen(PORT, () => {
  ...
  console.log(`🔁 Using ${RELAYER_URL ? 'external relayer' : 'EMBEDDED relayer'}: ${RELAYER_URL || 'localhost'}`);
